...
function newP(){var n=prompt('新预设名称:');if(!n||!n.trim())return;var s=extension_settings[EXT],k=n.trim().toLowerCase().replace(/\s+/g,'_');if(PRESETS[k]){toastr.warning('不能覆盖内置预设');return;}if(!s.customPresets)s.customPresets={};s.customPresets[k]={name:k,label:n.trim(),promptPrefix:s.promptPrefix||'',promptSuffix:s.promptSuffix||'',injectionPrompt:s.promptInjection.prompt||'',tagFormat:s.tagFormat,customRegex:s.promptInjection.regex};s.currentPreset=k;updatePDD();saveSettingsDebounced();toastr.success('新预设 "'+n.trim()+'" 已创建');}
